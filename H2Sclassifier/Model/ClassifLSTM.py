...
class ClassifLSTM(nn.Module):
  ...
  def __init__(self, hidden_size, num_layers, seq_len, batch_size, num_joints, NUM_CLASSES, bidirectional=True):
    super().__init__()
    ...
    self.lstm = nn.LSTM(5*num_joints, hidden_size, num_layers, bias=True, batch_first=True)
